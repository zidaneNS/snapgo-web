    ...
    <div id="service" className="flex flex-col items-center w-full gap-y-8 md:gap-y-12 pt-20 px-8 scroll-pt-20">
      ...
      <div className="flex flex-col md:flex-row gap-y-8 md:gap-x-12 w-full">
        {services.map((service, idx) => (
          <div key={idx} className="flex flex-col gap-y-4 md:gap-y-8 flex-1 rounded-md shadow-md cursor-pointer px-6 py-4 hover:scale-105 duration-300 pb-auto h-auto">
            <service.Icon className="size-12 text-yellow-400" />
            ...
            <p className="text-xl font-bold text-yellow-400">Starting from IDR {convertCurrency(service.price)}</p>
            <button className="text-center cursor-pointer py-2 px-4 rounded-md duration-300 border border-slate-200 shadow-md hover:bg-yellow-400">Get Quote</button>
          </div>
        ))}
      </div>
      <div className="flex flex-col items-center gap-y-4 md:gap-y-8 md:max-w-1/2 pt-8">
        ...
        <div className="flex flex-col md:flex-row items-center gap-y-8 md:gap-x-16">
